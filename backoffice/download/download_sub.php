			...
			<div class="content-header">
				<div class="container-fluid">
					...
					<ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">จัดการไฟล์ : <?php echo $dl_sub_type ?></li>
					</ol>
				</div>
			</div>
			<!-- Main content -->
                    ...
                    <div class="card shadow mb-4">
                        <div class="card-header">
                            <div class="text-right">
                                <a class="btn btn-primary btn-sm" href="./form_sub.php?stype=<?php echo $stype; ?>">อัพข้อมูล</a>
                            </div>
                        </div>
                        <div class="card-body">
                            <table id="example" class="table table-bordered table-sm">
                                <thead>
                                    ...
                                    <th width="150" class="text-center">ดาวน์โหลดไฟล์</th>
                                    <th width="150" class="text-center">จัดการ</th>
                                </thead>
                                <tbody>
                                    <?php foreach ( $dl_file_sub->where( "WHERE type='{$stype}'" )->orderBy( 'id', 'DESC' )->all() as $key => $value ): ?>
                                    <tr id="<?php echo $value['id']; ?>">
                                        <td class="align-middle">
                                            <?php if ( $value[selected] == 1 ): ?>
                                            <a href="<?php echo $value[url] ?>"><?php echo $value['topic']; ?></a>
                                            <?php else: ?>
                                            <a target="_blank" href="../../filesAttach/download/<?php echo $value['filename']; ?>"><?php echo $value['topic']; ?></a>
                                            <?php endif;?>
                                        </td>
                                        <td class="text-center align-middle">
                                            <?php if ( $value[selected] == 1 ): ?>
                                            <a target="_blank" href="<?php echo $value['url']; ?>" class="btn btn-success btn-sm"><i class="fa fa-link"></i> ดูลิงก์</a>
                                            <?php else: ?>
                                            <a target="_blank" href="../../filesAttach/download/<?php echo $value['filename']; ?>" class="btn btn-success btn-sm"><i class="fa fa-file"></i> ดูไฟล์แนบ</a>
                                            <?php endif;?>
                                        </td>
                                        <td class="text-center align-middle">
                                            <a href="./form_sub.php?sid=<?php echo $value['id'] ?>" data-toggle="tooltip" data-placement="bottom" title="แก้ไข" class="mx-1 btn btn-warning btn-sm text-white"><i class="fa fa-pen"></i></a>
                                            <a href="javascript:void(0)" data-delete="sub_file" data-id="<?php echo $value['id']; ?>" data-file="<?php echo $value['filename']; ?>" data-toggle="tooltip" data-placement="bottom" title="ลบ" class="mx-1 btn btn-danger btn-sm"><i class="fa fa-trash"></i></a>
                                        </td>
                                    </tr>
                                    <?php endforeach;?>
                                </tbody>
