<?php
	...
	if(empty($_GET['eid']))
	{
		$breadcrumb = 'สร้างหมวดหมู่';
	}
	else
	{
		$breadcrumb = 'แก้ไขหมวดหมู่ : '.$topic;
	}
	if($_SESSION['ssid']==1)
	{
		$where = '';
	...
	{
		$where = "AND user_id={$_SESSION['ssid']}";
	}
?>
<!DOCTYPE html>
<html lang="en">
			...
			<div class="content-header">
				<div class="container-fluid">
					...
					<ol class="breadcrumb">
						...
						<li class="breadcrumb-item active"><?php echo $ntopic; ?></li>
					</ol>
				</div>
			</div>
			<!-- Main content -->
						...
						<div class="card-header">
                            <div class="">
                                <a href="./form.php?type=<?php echo $type; ?>" class="btn btn-primary btn-sm">
                                    <small><i class="fa fa-plus"></i> เพิ่ม<?php echo $ntopic; ?></small>
                                </a>
                            </div>
                        </div>
						<div class="card-body">
                            <table id="example" class="table table-bordered table-sm">
                                <thead>
                                    ...
                                    <th width="150" class="text-center">ไฟล์แนบ</th>
                                    <th width="150" class="text-center">จัดการ</th>
                                </thead>
                                <tbody>
                                    <?php foreach ( $news->where( "where type=$type $where" )->orderBy( 'postdate', 'DESC' )->all() as $key => $value ): ?>
                                    <tr id="<?php echo $value['id']; ?>">
                                        <td class="align-middle"><a href="../../news-detail.php?id=<?php echo $value['id']; ?>"><?php echo $value['topic']; ?></a></td>
                                        <td class="text-center align-middle"><?php if ( !empty( $value['filename'] ) ): ?> <a target="_blank" href="../../filesAttach/news/<?php echo $value['filename']; ?>"><i class="fa fa-file"></i> ดูไฟล์แนบ</a> <?php else:echo '-';endif;?></td>
